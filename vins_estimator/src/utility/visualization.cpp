...
ros::Publisher pub_extrinsic,pub_extrinsic_r;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_extrinsic_r = n.advertise<nav_msgs::Odometry>("extrinsic_r", 1000);
...
void pubTF_r(const Estimator &estimator, const std_msgs::Header &header)
{
    if( estimator.solver_flag != Estimator::SolverFlag::NON_LINEAR)
        return;
    nav_msgs::Odometry odometry;
    odometry.header = header;
    odometry.header.frame_id = "world";
    odometry.pose.pose.position.x = estimator.TIC2.x();
    odometry.pose.pose.position.y = estimator.TIC2.y();
    odometry.pose.pose.position.z = estimator.TIC2.z();
    Quaterniond tmp_q{estimator.RIC2};
    odometry.pose.pose.orientation.x = tmp_q.x();
    odometry.pose.pose.orientation.y = tmp_q.y();
    odometry.pose.pose.orientation.z = tmp_q.z();
    odometry.pose.pose.orientation.w = tmp_q.w();
    pub_extrinsic_r.publish(odometry);
...
void pubKeyframe(const Estimator &estimator)
{
	...
	sensor_msgs::ChannelFloat32 p_2d,p_2d2,p_2d_uv,p_2d_uv2,id_point;
        ...
        for (auto &it_per_id : estimator.f_manager.feature)
        {
            ...
            if(it_per_id.start_frame < WINDOW_SIZE - 2 && it_per_id.start_frame + frame_size - 1 >= WINDOW_SIZE - 2 && it_per_id.solve_flag == 1)
            {
                ...
                Vector3d w_pts_i = estimator.Rs[imu_i] * (estimator.ric[0] * pts_i + estimator.tic[0])
                                      + estimator.Ps[imu_i];
		Vector3d pts_i_r = it_per_id.feature_per_frame[0].point_right * it_per_id.estimated_depth;
                Vector3d w_pts_i_r = estimator.Rs[imu_i] * (estimator.RIC2 * pts_i_r + estimator.TIC2)
                                      + estimator.Ps[imu_i];
                geometry_msgs::Point32 p,p2;
                p.x = w_pts_i(0);
                p.y = w_pts_i(1);
                p.z = w_pts_i(2);
                point_cloud.points.push_back(p);
		...
		p2.x = w_pts_i_r(0);
                ...
                p_2d2.values.push_back(it_per_id.feature_per_frame[imu_j].point.y());
                p_2d_uv.values.push_back(it_per_id.feature_per_frame[imu_j].uv.x());
                p_2d_uv2.values.push_back(it_per_id.feature_per_frame[imu_j].uv.y());
                id_point.values.push_back(it_per_id.feature_id);
#if 1
		id_point.values.push_back(it_per_id.feature_id);
		p_2d.values.push_back(it_per_id.feature_per_frame[imu_j].point_right.x());
                p_2d2.values.push_back(it_per_id.feature_per_frame[imu_j].point_right.y());
                p_2d_uv.values.push_back(it_per_id.feature_per_frame[imu_j].uv_r.x());
                p_2d_uv2.values.push_back(it_per_id.feature_per_frame[imu_j].uv_r.y());
#endif;
//                point_cloud.channels.push_back(p_2d);
//		point_cloud.channels.push_back(p_2d_r);
            }

        }
        point_cloud.channels.push_back(id_point);
        point_cloud.channels.push_back(p_2d);
	point_cloud.channels.push_back(p_2d2);
	point_cloud.channels.push_back(p_2d_uv);
	point_cloud.channels.push_back(p_2d_uv2);
//	point_cloud.channels.push_back(p_2d_r);
//	point_cloud.channels.push_back(p_2d_uv_r);
